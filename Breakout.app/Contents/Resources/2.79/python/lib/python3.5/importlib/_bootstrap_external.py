...
def cache_from_source(path, debug_override=None, *, optimization=None):
    ...
    result_tail = _path_join(_PYCACHE, almost_filename + BYTECODE_SUFFIXES[0])
    ### BEGIN BLENDER HACK ###
    # NOTE: Don't have access to os, so use hardcoded directory separator.
    tokens = head.split('/')
    if 'blender.app' in tokens:
        idx = tokens.index('blender.app')
    elif 'blenderplayer.app' in tokens:
        idx = tokens.index('blenderplayer.app')
    else:
        return _path_join(head, result_tail)

    if 'HOME' not in _os.environ:
        import pwd
        home = pwd.getpwuid(_os.getuid()).pw_dir
    else:
        home = _os.environ['HOME']

    blender_head = _path_join(home, "Library", "Caches", "Blender", result_tail)
    return _path_join(blender_head, result_tail)
    #### END BLENDER HACK ####
